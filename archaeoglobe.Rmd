...
plot_trends <- function(x, variable, color_theme = 'PuOr'){
  ...
  p2 <- x$trends[[1]] %>%
    ...
    scale_y_continuous(breaks = c(0,1), labels = c('None', 'Common')) +
...
plot_trends2 <- function(x, variable, color_theme = 'PuOr'){
  p1 <- x$trends[[1]] %>%
    gather(time, value, '-10000':'-150') %>%
    ggplot(aes(as.numeric(time) / 1000, value, 
               group = REGION_LAB, color = as.factor(cluster))) +
...
```{r, fig.width = 6.5, fig.height = 7.5, fig.cap = 'Global and regional trends in the presence of urban centers. (A) Global trend (all regions) with 95% confidence interval. (B) Regional deviations from global trend, clustered via k-means. (C) Map of the local deviations from the global trend, same clusters as in B.'}
trend_dat[7, ] %>%
  mutate(trends = map(trends, ~mutate(.,
                                 cluster = recode_factor(cluster, 
                                 `2` = '1', `6` = '2',`5` = '3', 
