...
q2 <- trend_dat[1,]$mod[[1]] %>%
    ...
    labs(title = 'B', x = 'Thousand years BP', y = 'Expertise') +
    theme_bw()

q3 <- trend_dat[2,]$mod[[1]] %>%
    plot(select = 0) %>% # selects the global trend
    .[1] %>%
    map(~tibble(time = .$x, fit = c(.$fit), se = .$se)) %>%
    .[[1]] %>%
    ...
    labs(title = 'C', x = 'Thousand years BP', y = 'Data Quality') +
    theme_bw()
...
q1 / (q2 | q3) /  q4 +plot_layout(heights = c(2,1,2))
