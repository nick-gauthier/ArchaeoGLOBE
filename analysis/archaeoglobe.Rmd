...
ggplot(aes(years_BP, perc, fill = land_use_category_consensus_assessments)) +
  geom_area(position = 'stack') +
  scale_fill_brewer(palette = 'Paired') +
  theme_minimal() +
  theme(legend.position = c(0.4, 0.85), #  x- and y- offsets from the bottom-left of the plot, ranging from 0 - 1.
    legend.text = element_text(size = 10),
    legend.key.size = unit(0.7, 'line')) +
    guides(fill = guide_legend(ncol = 2, title = 'Land-use category based on consensus assessments')) +
  labs(x = 'Years BP', y = 'Percent of all regions')
```
