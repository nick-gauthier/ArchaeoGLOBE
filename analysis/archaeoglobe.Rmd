...
```{r echo = FALSE}
# if there is a saved object on disk containing the model output, don't run the next chunk of code
# a simple-person's caching :) 
rerun_timeconsuming_analysis <- ifelse(file.exists("data/derived-data/trend_dat.Rda"), FALSE, TRUE)
```
...
```{r echo = FALSE}
# if trend_dat is already in the workspace, use it, if not, read it in from disk
# a simple-person's caching :)
