...
```{r hyde-processing}
## need to use rcurl or something to download hyde directly
hyde_crops <- list.files('../HYDE/', full.names = TRUE, recursive = TRUE) %>%
  .[str_detect(., 'cropland')] %>% # get just the cropland files
  map(raster) %>%
  .[c(9:7,1:6)] %>% # hacky solution to fix later, sensitive to OS settings
  brick %>%
