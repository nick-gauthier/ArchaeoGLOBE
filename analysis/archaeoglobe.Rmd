...
```{r echo = FALSE}
# A simple type of caching...
# Do we want to download the raw data, or
# load a previously saved result from disk, or
# download a previously saved result from a repository?
# default is not run, then check if there is a saved file, and use that, or download
...
if(rerun_time_consuming_download) {
  message('running the download code, this may take 30-50 min...')
  # go to the next chunk of code
} else {
  # check if there is a local file and if so, load it
  if(file.exists('data/derived-data/hyde_crop_prop.RDS') & file.exists('data/derived-data/kk_anthro_prop.RDS')) {
    # the files exist on the local disk, so just read them in
    message('Loading previously saved data from disk...')
    hyde_crop_prop <- readRDS('data/derived-data/hyde_crop_prop.RDS')
    kk_anthro_prop <- readRDS('data/derived-data/kk_anthro_prop.RDS')
  } else {
  # we don't want to run the download code, and the result don't exist locally,
  # so download
  message('Downloading previously saved data ...')
  hyde_crop_prop <- osf_retrieve_file('#####') %>% osf_download('data/derived-data/hyde_crop_prop.RDS')
  kk_anthro_prop <- osf_retrieve_file('#####') %>% osf_download('data/derived-data/kk_anthro_prop.RDS')
  message('Loading the data downloaded from osf.io...')
  hyde_crop_prop <- readRDS('data/derived-data/hyde_crop_prop.RDS')
  kk_anthro_prop <- readRDS('data/derived-data/kk_anthro_prop.RDS')
  writeLines(paste0('hyde_crop_prop.RDS downloded from https://osf.io/#####/ on ', Sys.Date()), con = 'data/derived-data/README.md', sep = '')
  writeLines(paste0('kk_anthro_prop.RDS downloded from https://osf.io/#####/ on ', Sys.Date()), con = 'data/derived-data/README.md', sep = '')
  message('Done.')
  } 
}
```
...
writeLines(paste0('hyde_crop_prop generated on ', 
                  Sys.Date() , ' from archaeoglobe.Rmd at git commit ',
                  this_commit$sha, ' made by ',
                  this_commit$author$name, ' on ',
                  this_commit$author$when, " with the message '",
...
writeLines(paste0('kk_anthro_prop generated on ', 
                  Sys.Date() , ' from archaeoglobe.Rmd at git commit ',
                  this_commit$sha, ' made by ',
                  this_commit$author$name, ' on ',
                  this_commit$author$when, " with the message '",
...
# upload the hyde derived  data
osf_retrieve_node('gcfs6') %>% 
  osf_upload(path = 'data/derived-data/hyde_crop_prop.RDS', 
             overwrite = TRUE) 

# upload the kk10 derived data
osf_retrieve_node('gcfs6') %>%
  osf_upload(path = 'data/derived-data/kk_anthro_prop.RDS',
             overwrite = TRUE)
