...
```{r}
ag_join <- read_csv('../hyde_archaeoglobe_dataset_2018_08_31/Ag_Join.csv') %>%
  select(-DIFF, -AG_W_InExAg) %>%
  gather(type, time, 2:5) %>%
  mutate(time = na_if(time, 'Never'),
         time = if_else(str_detect(time, 'BP'), parse_number(time) * -0.001, 
                        (parse_number(time) - 1950) * 0.001)) %>%
  separate(type, c('source', 'level', NA)) %>%
  mutate(level = if_else(level == 'CW', 'Common (> 1% land area)', 
...
ag_join %>%
  ...
  scale_fill_viridis_c(name = 'Thousand years BP', guide = 'colorbar') +
  geom_sf_text(aes(label = region_label), nudge_x = 500000, size = 2.5, hjust = 0, expand = TRUE) +
  coord_sf(datum = NA) +
  theme_void() +
...
ggplot(onset_difference, aes(diff, group = diff)) +
  ...
  scale_fill_distiller(palette = 'PuOr', limits = c(-6,6)) +
  facet_wrap(~level) +
  theme_minimal() +
  labs(x = 'Onset difference (Thousand Years)') +
  coord_flip()
...
onset_difference %>%   
  ...
  scale_fill_distiller(name = 'Onset', guide = 'colorbar', palette = 'PuOr', direction = 1, limits = c(-6, 6)) +
  geom_sf_text(aes(label = region_label), nudge_x = 500000, size = 2.5, hjust = 0, expand = TRUE) +
  coord_sf(datum = NA) +
  theme_void() +
  facet_wrap(~ level) +
