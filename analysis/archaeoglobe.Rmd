...
a <- regions %>%
  ...
  #coord_sf(xlim = c(-16651340, 16653290), ylim = c(-8411026, 8374612)) +
...
b <- ggplot(response_counts, aes(n, group = n)) +
  ...
  theme_classic() +
  labs(title = 'B', x = 'Contributors', y = 'Regions')

a + b + plot_layout(ncol = 1, heights = c(2,1))
...
ggplot(aes(time, fit, group = type)) +
  ...
  labs(title = 'A', x = 'Thousand years BP', y = 'Land-use extent\n') +
  theme_bw() +
  facet_wrap(~type)
...
ggplot(aes(time, fit, group = type)) +
  ...
  labs(title = 'B', x = 'Thousand years BP', y = 'Archaeological \nknowledge') +
  theme_bw() +
  facet_wrap(~type)
