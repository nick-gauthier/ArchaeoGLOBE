...
plot_trends_knowledge <- function(x, variable, tag){
  ...
  rt2 <- x$trends[[1]] %>%
    ...
    scale_y_continuous(breaks = c(0,1), labels = c('Low', 'High')) +
    facet_wrap(~cluster, nrow = 1, 
               labeller = labeller(cluster = c(`1` = 'Earlier', `2` = '', `3` = '', `4` = '', `5` = '', `6` = 'Later'))) +
    labs(x = 'Thousand years BP', y = 'Extent') +
    theme_minimal() +
    theme(strip.background = element_blank())
...
k1 <- trend_dat[1, ] %>%
  mutate(trends = map(trends, 
                      ~mutate(., cluster = recode_factor(cluster, 
                                 `1` = '1', `2` = '2',`3` = '3', 
                                 `5` = '4', `4` = '5', `6` = '6')))) %>%
  plot_trends_knowledge('Expertise', tag = 'C') + 
  labs( y = 'Expertise')
...
k2 <- trend_dat[2, ] %>%
  mutate(trends = map(trends, ~mutate(., 
                                 ...
                                 `3` = '1', `2` = '2',`6` = '3', 
                                 `1` = '4', `5` = '5', `4` = '6')))) %>%
  plot_trends_knowledge('Data Quality', tag = 'D') + 
  labs(y = 'Quality')
...
(g2 + sites_plot + labs(tag = 'B')) / (k1 | k2) + plot_layout(heights = c(1, 2))

ggsave('figures/trends_map_knowledge.png', height = 8, width = 12)
```
