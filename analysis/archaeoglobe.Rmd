...
```{r}
archaeoglobe <- read_csv('data/raw-data/ARCHAEOGLOBE_PUBLIC_DATA.csv')
regions <- st_read('data/raw-data/Simplified_Regions2.shp', quiet = TRUE)
```
...
rerun_timeconsuming_analysis <- FALSE # FALSE for do not run the modelling code

if(rerun_timeconsuming_analysis) {
...
```{r, eval = rerun_timeconsuming_analysis}
...
```{r}
get_g_data1 <- get_g_prefix("HUNT")
```

```{r}
library(mgcViz)
b <- getViz(get_g_data1)
```
...
g1a <-   o + 
   l_fitLine() +
   l_ciLine(mul = 2) +
  scale_x_continuous(breaks = c(-10, -8,-6,-4,-2,0)*1000,
...
```{r}
plot_gam_out <- invisible(mgcv::plot.gam(x = get_g_data1, select = 0))
```

```{r}
g1data <- get_fit(plot_gam_out)
```
...
```{r gridxtra, echo=FALSE}
library(gridExtra)
png('figures/global_trends.png', height = 12, width = 12, units = "in", res = 300)
grid.arrange(grobs = list(g1, g2, g3, g4, g5 ,g6), ncol = 2, nrow = 3)
dev.off()
```
...
(g1 + g2) / (g3 + g4) / (g5 + g6)
dev.off()
knitr::include_graphics("figures/global_trends.png")
```
