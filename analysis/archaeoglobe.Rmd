...
ggplot(onset_difference, aes(diff, group = diff)) +
  ...
  theme(legend.position = c(0.80, 0.95, 
        ...
        legend.text=element_text(size=6), 
        legend.text.align = 1,
        legend.kdirection="horizontal",
        legend.ey.size = unit(0.5,"line"))
 +
...
onset_difference %>%   
  ...
  geom_sf_text(aes(label = region_label), 
               nudge_x = 500000, size = 2.5, hjust = 0, expand = TRUE) +
  coord_sf(datum = NA) +
  theme_void() +
  facet_wrap(~ level) +
...
(aag_join_maps  / onset_difference_maps / onset_difference_barplot) + 
